...
function configureCollections() {
    ...
    return resolver.coWrap(function*() {
        var path = resolver.pathTo(resolver.CONSTANT().SERVER_COMMON_MODELS_PATH);
        //logger.debugTerminal('Reading ', path);
        var exists = yield resolver.getFacade('fs').exists(path);
        if (!exists) {
            logger.warnTerminal(path, ' should exist.');
        }
        var arr = yield resolver.getFacade('fs').readdir(path);
        //logger.debugTerminal(arr);
        arr.forEach(fileName => {
            var module = resolver.require(resolver.CONSTANT().SERVER_COMMON_MODELS_PATH, fileName);
            ...
            if (!module.name || !module.def || !module.configure) {
                ...
                logger.debugTerminal("Model", module.name, 'configured OK');
            }
        });
