"use strict"
var reloadRequire = require('require-reload')(require);
...
module.exports = {
    ...
    urlencode: urlencode,
    mdToHtml: mdToHtml,
    reloadRequire: reloadRequire,
    getFilePathsTreeRecursive: getFilePathsTreeRecursive,
    getFilesTreeRecursiveAsync: getFilesTreeRecursiveAsync
};

function getFilesTreeRecursiveAsync(absolutePath) {
    return new Promise((resolve, reject) => {
        var f = readDirFiles.readSync(absolutePath);
        f = normalizeFilesTreePreservePath(f);
        return resolve(f);
    });
}
...
function concatenateAllFilesFrom(path, opt) {
    ...
    if (opt.ext) {
        opt.ext = opt.ext.map(ext => ext.replace('.', ''));
    }
    Object.keys(files).sort().forEach(k => {
        //console.log('concat ',k);

        if (opt.ext && opt.ext.length > 0) {
            for (var x in opt.ext) {
                if (k.indexOf('.' + opt.ext[x]) !== -1) {
                    console.log('Concat file...', k);
                    rta += files[k];
                    return;
                }
                else {
                    console.log('Concat file...', k, ' Skip because validation', '.' + opt[x]);
                }
            }
        }
        else {
...
function getFilePathsTreeRecursive(absolutePath) {
    var files = normalizeFilesTreePreservePath(readDirFiles.readSync(absolutePath));
...
function copyFilesFromTo(FROM_PATH, DEST, opt) {
    ...
    var rta = new Promise((resolve, error) => {
        ...
        var filePath, rawContent, compiledContent, fullpath;
        ...
        Object.keys(files).forEach(k => {
            ...
            if (opt.filter) {
                ...
                var rta = opt.filter(fileName, filePath);
                if (!rta) {

                    //console.log('=============TRACE', 'TPL-SKIP', fileName);
            ...
            if (opt.formatPathHandler) {
                filePath = opt.formatPathHandler(filePath);
            }

            //Accessor to change the final path
            var handlerScope = {
                setFinalPath: (p) => filePath = p
            };

            if (opt.formatContentHandler) {
                try {
                    //console.log('=============TRACE', 'TPL-FORMAT', filePath);
                    compiledContent = opt.formatContentHandler.apply(handlerScope, [rawContent, filePath]);
                }
                catch (e) {
                    console.log('=============TRACE: utils copyFilesFromTo write error', e.substring(0, 50));
                    hasErrors = true;
                }
            }
            else {
