...
function build() {
	...
	gulp.task('sass', function() {
		var mainPath = path.join(process.cwd(), 'src/client', resolver.env().APP_NAME, '/css/main.scss');
		if (!fs.existsSync(mainPath)) {
			return logger.warnTerminal('main.scss not found at ', mainPath);
		}
		var options = {};
		...
		if (resolver.env().PROD) {
			...
			return gulp.src(mainPath)
				...
				.pipe(gulp.dest(path.join(process.cwd(), resolver.env().DEST, 'css')));
		}
		else {
			return gulp.src(mainPath)
