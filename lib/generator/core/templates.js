...
var logger = resolver.logger().get('GENERATOR', 'TEMPLATES');
...
const PROD = resolver.env().PROD;
const APP_NAME = resolver.env().APP_NAME;
const COMMON_PARTIALS_PATH = path.join(process.cwd(), 'src/client/common/partials');
...
function watchPartials() {
    ...
    heUtils.watch(COMMON_PARTIALS_PATH, build);
...
function isValidStaticFilePath(filePath) {
...
function getCompiledPath(staticFilePath) {
    if (!isValidStaticFilePath(staticFilePath)) return null; //invalid path
    var basePath = staticFilePath.substring(0, staticFilePath.lastIndexOf('/'));
    var fileName = staticFilePath.substring(staticFilePath.lastIndexOf('/'));
    var ext = fileName.substring(fileName.lastIndexOf('.'));
    var fileNameWithoutExt = fileName.substring(0, fileName.lastIndexOf('.'));
    if (ext === '.md') {
        staticFilePath = staticFilePath.replace('.md', '.html');
    }
    if (fileName.indexOf('index') === -1) {
        return path.join(basePath, fileNameWithoutExt, 'index.html'); //hello.html becomes hello/index.html
    }
    else {
        staticFilePath = staticFilePath.substring(0, staticFilePath.indexOf('index.')) + 'index.html'; //normalize
    }
...
function buildTemplates() {
    return new Promise((resolve, error) => {
        ...
        logger.debugTerminal('currentPages', heFirebase.currentPages);
        var hotReload = false;

        logger.debugTerminal('From', SRC.substring(SRC.lastIndexOf('/')), 'to', heConfig().output().substring(heConfig().output().lastIndexOf('/')), heConfig().i18n_config.current);
        heUtils.copyFilesFromTo(SRC, heConfig().output(), {
            filter: (fileName, filePath) => {
                return isValidStaticFilePath(filePath);
            },
            ...
            formatContentHandler: function(raw, relativePath) {
                ...
                if (fileName.indexOf('index') === -1) {
                    ...
                    logger.debugTerminal('OUT', path.join(basePath, fileNameWithoutExt, 'index.html'));
                }

                logger.debugTerminal('relativePath', relativePath);
                ...
                try {
                    ...
                    rta = raw;
                    logger.warn(relativePath, 'Fail with error', e.stack);
                    throw e;
                }
                ...
                if (heFirebase.currentPages) {
                    var match = heFirebase.currentPages.filter(c => c.indexOf(fileNameWithoutExt) !== -1);
                    if (match.length == 1) {
                        heFirebase.sendHotHtml(match[0], rta);
                        hotReload = true;
                    }
                    else {
                        if (match.length !== 0) {
                            logger.warnTerminal('A file match more than one current page', fileNameWithoutExt, heFirebase.currentPages);
                        }
                    }
                }
            ...
            if (hotReload) {
                logger.debugTerminal('HOT');
            }
            emit('build-success', {
                wasHotReload: hotReload
            });
