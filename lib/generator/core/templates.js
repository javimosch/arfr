        ...
        heUtils.copyFilesFromTo(SRC, heConfig().output(), {
            ...
            formatContentHandler: function(raw, relativePath) {
                ...
                if (ext === '.md') {
                    ...
                    var templateConfig = path.join(process.cwd(),SRC, basePath, fileNameWithoutExt + '.js');
                    var templateName = require(templateConfig).templateName;
                    var templateHtml = Handlebars.compile("{{>" + templateName + "}}")(heConfig());
