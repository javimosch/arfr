...
Handlebars.registerHelper('i18n', function(context, abs) {
    ...
    //if (heConfig().i18n) {
    //  if (heConfig().i18n[context]) {
    //    if (heConfig().i18n[context][current]) {
    //      raw = heConfig().i18n[context][current] || raw;
    //    }
    //}
    //}
    //}
        ...
        co(function*() {
            ...
            for (var x in keys) {
                ...
                if (isAfter) {
                    ...
                    logger.debug('TEMPLATES: Using firebase version for ', name);
                }
                else {
...
function compileVendorCustom(opt) { //{{root}}
    ...
    if (heParser.hasSection(sectionName, raw)) {
        ...
        if (opt.middleWare) {
            logger.debug(sectionName, ' before middleware', compiledCode.length);
            compiledCode = opt.middleWare(compiledCode);
            logger.debug(sectionName, ' after middleware', compiledCode.length);
        }
        heUtils.createFile(buildPath, compiledCode);
        logger.debug('bundle output ' + buildPath + ' success at '); // + new Date());
...
function buildTemplates() {
    return new Promise((resolve, error) => {
        ...
        logger.debug('Copy from ', SRC, 'to', heConfig().output(), 'LANG:', heConfig().i18n_config.current);
        heUtils.copyFilesFromTo(SRC, heConfig().output(), {
            ...
            formatContentHandler: (raw, path) => {
                ...
                //console.log('BUILDER:', path, 'format handler');
                ...
                try {
                    ...
                    rta = raw;
                    console.log('BUILDER WARN templates compiling fail', e);
                    throw e;
                }
            ...
            logger.debug('build ' + (res.ok ? 'success' : 'with errors') + ' at ' + new Date());
            emit('build-success');
            resolve();
        })
    });
