...
function build() {
	...
	//logger.debug(' MAIN Build JS Path',PATH);

	var raw = heUtils.concatenateAllFilesFrom(PATH, {
		debug: false
	});

	//logger.debug('build before chars len', raw.length && raw.length || null);
	...
	if (process.env.PROD.toString() == '1') {
		...
		build_next(r.code);
		logger.debug('build after chars len', r && r.code && r.code.length || null);
	}
	else {
		build_next(raw);
	}
	...
	function build_next(_raw) {
		...
		logger.debug('build ' + g.destFilename + ' success at ' + new Date());
		emit('build-success');
	}
}
